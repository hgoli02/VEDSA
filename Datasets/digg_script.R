...
data_ <- read.csv("digg_seisimic.csv")
index_ <- read.csv("index.csv")

num_cascades = max(data_[, 2])
len_data = length(data_[, 2])

burst_min_len = 800
non_burst_max_len = 700
Observation_len = 24 * 60 * 60 
time_bin = 7200
prediction_length = 30 * 24 * 60 * 60
k = 50
...
for (i in c(1:length(selected_cascades))) {
  ...
  pred <- pred.cascade(pred.time, infectiousness$infectiousness, cascade_times, followers, n.star = 15.61)
...
#j=1
#pred.time <- seq(0, 30 * 24 * 60 * 60, by = 7200)
#obs =  2 * 24 * 60 * 60
